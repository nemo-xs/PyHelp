...
def max3(z, a):
    """значения здесь - могут быть условные (как неизвестные) или известными.
    Они никак не связаны с конкретными переменными"""
    if z > a:
        return z,   # вернуть
...
Чтобы не имопртировать все подряд, можно через метод __all__ сформировать
