...
import os
import time
from multiprocessing import Process  # стандартный модуль, импорт класса
...
Другими словами, процесс — это набор инструкций, которые выполняются
последовательно. """
...
""" #? вариант мультипроцессорности только для linux
foo = 'bar'
...
# ? вариант мультипроцессорности (запуск через if __name ==__main__)
# def f(name):
#     print("hello", name)
...
# ? создание методом ООП (запуск также через if __name == __main__)
...
# ? через создание экземпляра класса Thread
def t(name):
...
"""Создается экземпляр класса Thread, для запуска в отдельном потоке
в него передается функция (или часть кода) и аргументы для этого кода (если они
им принимаются)"""
th.start()  # запуск отдельного потока
th.join()  # ждет исполнения кода и завершает поток


# ? через создание дочернего класса от Thread (наследование)
class PrintProcess(Thread):
    def __init__(self, name):
        super().__init__()
        self.name = name

    def run(self):  # переопределени стандартного метода run
        print("Hello", self.name)


p = PrintProcess("Mike")
p.start()  # запуск отдельного потока
...
# -----------------------------------------------------------------
""" #? Блокировки можно создавать и без контекстного менеджера with.
...
# region #* Глобальная блокировка интерпретатора GIL --------------------------
