...
# ? Целесообразно при пакетном импорте через __init__.py (см ниже)
...
Пути лучше указывать относительные #? from .module_name import *, где "." перед
...
#? __all__ = module_1.__all__ + module_2.__all__
...
# ? множественное присваивание работает слева направо:
a, b, c = 80, 90, 100  # Присвеоение значений по типу кортеджа
a, (a, c) = 90, (80, 100)  # а будет = 80, так как вычисляется слева направо
"""
#? Ключевые аргументы, аналогично позиционным,можно упаковывать/распаковывать
...
# ? множественное присваивание через *, ** также работает в циклах (for).
print(center)
...
# ? Предикаты (проверка является ли строка):
...
# ? совмещение с преобразованием объекта в строки:
...
# ? f-string тоже самое, что format только без вызова метода format:
...
# ? использование именованых slice. Такой же принцип для всех итеррируемых
...
# ? запись результатов работы скрипта в файл
# py имя_файла.py > log.txt

"""#? Поток Python соджержит 3 системных файла. sys.stdin - поток входа,
