...
import pprint  # библиотека красивого вывода
...
всех/или не всех находящихся там модулей. Пакеты могут быть вложенными.
Плюсы такого метода, не надо прописывать пути импорта. Пакет импортируется по
имени папки, внутри него __init__.py содержит необходимые импорты уже с путями.
"""
pprint.pprint(sys.version)  # печать версии системы из модуля sys через модуль
# pprint
...
#   not логическое "нет"
#   and логическое "и"
#   or  логическое "или". Ленивое выражение. Если то что перд or выполняется,
#       дальше интерпритатор не проверяет.
...
print(s)
print("{} хороший товарищ {}".format("Вася", "Пети"))
