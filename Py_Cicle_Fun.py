...
# region #* оператор else в исполняемых циклах (while, for и тд.) -------------
...
он продолжает работу с того места где закончил, а не начинает все заново. То
есть после вызова, его локальные переменные сохраняют свое состояние, и могут
быть вызваны с того места, где закончили.
...
def pr(one, two, *args):  # Внутри скобок передаваемые значения (переменные).
    ...
    # сдалеть функцию сложения, а сколько элементов будет передано не известно
