...
# ?Используется как тернарный оператор. Принцип записи как у list comprihation
...
# ? else можно опускать, то есть не писать. Здесь перед pass, есть else:
for i in range(3):
    if i == 2:
        print(i)
...
"""#? Сопрограмма, это программа с несколькими точками входа. ----------------
...
def coroutine(g):
    """# ? Для скрытия деталей реализаци запуска сопрограммы, ее можно скрыть
...
def generator_range(first, last):
    """#? оператор yield from делегирует выполнение другому генератору)
