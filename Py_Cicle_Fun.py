...
# region #* if и While --------------------------------------------------------
print("Циклы")
"""Оператор if используется как проверка на True какого то утверждения. Писать
чему равно то что находится в if не надо. Если по логике оно будет равно True
то сработает без уточнения."""
if a:  # если а = True (то есть просто существует).
    pass

# _Используется как тернарный оператор. Принцип записи как у list comprihation
# используется для сокращения записи, условие непосредственно в функции
print("Wow" if x % 2 == 0 else "Wuuuu")
# -----------------------------------------

"""Цикл if - ветвление. Если так, то сделать то то.. Иначе - сделать другое"""
...
# и не исполняется первичное условие, то #_ 1 раз исполняется else
...
# _ вместо 'words' вставляется любой итерируемый объект, то есть все, что можно
...
# _ Пример использвания цикла for для выборки значений из списка
...
# _ https://stepik.org/lesson/24464/step/2?unit=6769


def control_iterator():
    """Если нужна контролируемая итерация, нужно использовать: (см yield)"""
    yield from range(10)


# * list comprihation ---------------------------------------------------------
# _ Подобная запись в 2 раза быстрее варианта через цикл. Поскольку нет .append
# * (values) = [(expression) for (value) in (collection) if (condition)]
# _ list_res = [x for x in list]
...
# _ list_res = [x * x for x in list if x > 0]
...
естественным путем, не через break"""
...
Для того чтобы правильно комбинировать исполнение кода, кроме break есть
#** оператор continue. Он не полностью прерывает исполнение цикла, а пропускает
все что следовало за ним, и отправляет исполнение на новый цикл
"""
"""#_ пример использования - в банкомате. Запускается цикл проверки введеного
...
for i in range(5):
    ...
    if i == 2:  # поменять на 6-ку и цикл не исполнится, перейдя в else
        print(i)
        break
else:  # // else в данном случае определен не для if, а для for (отступы)
    print('This is the end')

# _ else можно опускать, то есть не писать. Здесь перед pass, есть else:
for i in range(3):
    if i == 2:
        print(i)
    pass  # else не указано, но оно подразумевается по отступам.

"""В блоке try, else исполняется только в том случае, если исключения не было.
То есть когда то что пробовали сделать в try получилось и оно сработало, идем
в else.. Если же возникло исключение, то else игнорируется и идет переход в
except, и, если оно есть - finaly.
При этом в блоке else, except уже не срабатывает, в нем ошибки не ловятся.
"""
# endregion -------------------------------------------------------------------

# region #* with контекстный менеджер ----------------------------------------
"""Существует для управления рессурсами.. Например для того, чтобы не забыть
закрыть открытый файл, или еще что то подобное.
Реализация очнована на 2-х методах. __enter__ иниициирует контекст, например
захватывает файл или еще что то.. Полученное значение записывается как as.

Семантика with:
with(для) open(какого_то_действия) file.txt(с объектом) as r(названным как r):
    сделать то то и то то

после окончания блока контекстного менеджера вызывается метод __exit__, после
чего файл закрывается. Если в процессе исполнения вылезло исключение, метод
__exit__ его подавляет.
Расширение для with - import contextlib
.closing - закрытие соединения
.redirect_stdout - перехватывает вывод в стандартный поток
.suppress - подавляет исключения указанных типов
.ContextDecorator - класс позволяющий использовать менеджеры контекста как
декораторы (через наследование).
.ExitStack - позволяет работать в одном with с многими файлами
"""
# endregion -------------------------------------------------------------------

# region #* Функция map() как частная альтернатива цикла for ------------------
# _ map(function, *iterible)
...
итеррируемой последовательности. #_ Фактически это цикл for для функций.
...
# _ функция map может быть легко замен на простой цикл for следующей записью:
new_lister = [string.upper() for string in lister]  # это нагляднее map()
print(new_lister)
# _ все указанные результаты полностью идентичны. Везде на выходе список.
...
# _ Как и map(), можно использовать с lambda()"""
...
# _ после этого передает полученный результат вызывающей функции
...
""" Как только в теле функции появлется yield - это уже не функция а генератор.
...
Допустим, необходимо генерировать и итерировать очень большой список из
1,000,000,000,000 строк.
Такой объект тяжело итерировать, поскольку он всегда должен быть загружен в
память. После того, как в язык были добавлены генераторы, появилась возможность
с минимальными затратами создать итератор, которому не нужно генерировать всю
последовательность за раз. Вместо этого достаточно иметь память для хранения
только одного объекта в каждый момент времени. А при следующем вызове yield,
будет сгенерирован следующий объект.
...
# _ https://www.youtube.com/watch?v=ZjaVrzOkpZk
...
def accumulyator():
    ...
    total = 0
    while True:
        """Когда yield стоит справа от '=', вызывается метод send, который
        отправляет то, что стоит справа от yield (total) и записывает это в
        переменную слева от yield (value).
        При следующем вызове через .send(args) передаваемый аргумент
        подставляется на место total, и записывается в value"""
        value = yield total
        print("send: ", value)
...
generat = accumulyator()  # присваивание функцию переменной. На выходе ничего.
next(generat)  # первый вызов генератора. Проходит до yield, отправляет туда
# None и возврат в основной код. Генератор готов к работе (к приему значений)
print("Accumulated: ", generat.send(1))  # вызов с передачей значения через
# метод .send(), который передается в точку цикла откуда он вренулся (yield).
# Присаивает полученное значение value. Цикл проходит дальше с переданным
# значением value, прибавляется к total, доходит до yield и значение
# выплевывается в основной код программы
print("Accumulated: ", generat.send(3))  # повторный вызов как выше..

""" При работе с yield использхуются следующие методы:
.send() отправить на вход генератора yield
.throw(ExceptionName, 'текст') кидает выбранное исключение
.close() - закрывает генератор, кидает в него GenratorExit"""

"""#_ Сопрограмма, это программа с несколькими точками входа. ----------------
Поддерживает остановки и продолжение с сохранением состояния. Реализуется как
'генератор наоборот'. Пример:"""


def grep(pattern):
    """Принимает некий патерн, и ожидает запрос. Если паттерн встречается в
    запросе, он печатает эту строчку
    """
    print("Looking for {!r}".format(pattern))  # печать при инициации
    while True:
        """После запуска генератора он доходит до yield принимает None и
        передает инициативу в основной код. При вызове через send, присваивает
        line аргументы send и проверяет через if наличие патрена в строке"""
        line = yield
        if pattern in line:
            print(line)


gen = grep("Gosha!")  # присваивание функции и передача в него патерна

# запускаем генератор, вывод первого print("Looking..."). Доходим до yield,
# передаем инициативу в основной код
next(gen)
# первый вызов не содержит "Gosha!", поэтому строка не печатается
gen.send("This line dosn`t have \
         what we`re looking for")
# во втором вызове есть искомый патерн "Gosha!", print(line) "This one does..."
gen.send("This one does. Gosha!")
# ------------------------------------------------------------


def coroutine(g):
    """# _ Для скрытия деталей реализаци запуска сопрограммы, ее можно скрыть
    через использование декоратора, позволяющего в последствии не использовать
    next для активации генератора"""
    @functools.wraps(g)  # обертка
    def inner(*args, **kwargs):
        """Создает внтуренний inner, который делает только 1-но. вызывает next
        и возвращает значение обратно.
        *args, **kwargs не всегда работают. Лучше указать то, что действительно
        ожидается в качестве аргументов функции (см тему декораторов)"""
        gen = g(*args, **kwargs)
        next(gen)
        return gen
    return inner
...
# оборачивание grep декоратором coroutine. Правильней сразу обернуть через
# @coroutine саму функцию(вверху). Если сделать так, то строка не нужна
grep = coroutine(grep)

gen = grep("Gosha!")  # присваивание функции с передачей искомого аргумента
gen.send("One more line for Gosha!")  # send в функцию сопрограммы (grep)

# --------------------------------------------------------------


def generator_range(first, last):
    """#_ оператор yield from делегирует выполнение другому генератору)
    таким образом позволяет, например, производить контролируемую итерацию
    последовательностей, производя какие либо действия с полученными значениями
    между циклами генератора"""
    for i in range(first, last):
        yield i


def generator_range(first, last):
    """полностью аналогичная функция как выше. При этом также доступны методы
    .send(), .throw() и .close()"""
...
try:  # пробуем
    ...
    """Exvaption - станадартный (самый обширный) обработчик исключений.
    BaseException - родитель для Exception. Его использовать не желательно,
    поскольку он включает в себя системные ошибки, Ctrl С и т.п."""
...
# _ Только если ошибка не возникла, исполняeтся следующий за ним блок else
else:
...
finally:  # _ блок который запускается в любом случае. #* он важнее даже return
...
""" raise команда вызывающая исключение, со ссылкой к классу исключений. В
классе исключений BaseException есть много подклассов. Можно писать свои, но
свои исключения должны быть дочерними по отношению к BaseExceprion, а вернее от
простого Exceprion, поскольку он является дочерним, и не включает системные
ошибки """
# * exit()
...
def max2(z, a):   # значения где z и a это аргументы
    res = float('inf')  # указание атрибута как бесконечно большого числа
    res = float('-inf')  # указание атрибута как бесконечно малого числа
    try:
...
def bounded_min(first, *args, lo=float("-inf"), hi=float("inf")):
    """Сравнение множества значений с заданием максимума и минимума диапазона
    сравнения """
    res = hi
    for arg in (first, ) + args:
        """поскольку args это кортедж, для конкотинации first тоже делаем
        кортеджем, поэтому (first, )"""
