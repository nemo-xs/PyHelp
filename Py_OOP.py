...
# * инкапсуляция ---------------------------------------------------------
# TODO: найти что значат атрибуты заканчивающиеся подчеркиванием и др. услов-ти
...
if __name__ == "__main__":
    ...
    что ниже этой записи не импортируется, но запускается если запускается сам
    файл.
    Часто используется в ООП, где весь код это классы и функции, и лишь одна
    главная функция main собирает в себя весь написанный в функциях, классах и
    объектах код. Если используется для этого, то стоит в самом конце файла
    """
...
# Декоратор, это объявление чего либо, например, что данный метод стический...
...
class Rectangle:  # создаем класс прямоугольник с 2-мя сторонами
    ...
    def __repr__(self):  # еще один метод конструктор. Существует 2 похожих..
        # // __repr__ и __str__. Оба переводят значения в cтроку.Разница в том,
