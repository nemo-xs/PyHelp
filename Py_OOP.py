...
# @ не был указан непосредственно перед описанной функцией.
# Еще вариант вызова:
...
которые есть непосредственно в самом классе (выше). Привязан только к самому
классу (скобки пустые. там нет self)
...
class Complex:  # Странный конечно пример, на методе составных чисел
    def __init__(self, real=0.0, imreal=0.0):
        ...
        Данный метод вызывается автоматически при создании объекта класса, для
        иниициализации изначального состояния данного объекта. задает его
    ...
    def __str__(self):
        ...
        значения в cтроку. Вшит в функцию print"""
    ...
    def __eq__(self, other):
        """метод позволяющий сравнивать объекты, через == как is, not is"""
...
# region #* Инкапсуляция. Управление доступом к аттрибутам __getattribute__----
...
class Horse(Animal):
    def __init__(self):
        """вызов атрибута базового класса и его переопределение"""
        super().__init__()
        self.can_run = True
