...
# ? Все создаваемые классы Python являются наследниками класса object.
...
# ? Класс - это тип объектов. В пайтоне тип и класс это одно и тоже
# ? Объект - это данные + код, то есть данные обернутые некоторым кол-м функций
# ? Метод - функция принадлежащая к конкретному классу (созданная внутри него)
# методы вызываются через "." пример: имя_объекта_класса.метод(), либо сам
# объект класса "." имя метода(). Пример: "Hello world".метод()
# ? Параметры (атрибуты) класса, - локальные переменные внутри него
# ? Аргументы, это то что передается вместо параметров при создании объектов
...
"""
#* SOLID
#? Single - Принцип единственной ответственности. Каждый класс отвечает
...
#? Open/close - Принцип открытости/закрытости. Классы должны быть открыты для
...
#? Liskov Barbara - функции, которые используют базовый тип, должны иметь
...
#? Interface segregation - принцип разделения интерфейса. Клиенты не должны
...
#? Dependency invesion - модули верхних уровней не должны зависеть от модулей
...
class Goat:  # класс Коза (#* имена классов с Большой буквы)
    ...
    def show(self):
        ...
        """
        print(self.age)
# ? созданный в данном примере класс коза, позволит создать множество коз, у
...
# ? через () происходит вызов конструктора объекта класса и создается объект
...
по умолчанию.
"""
# ? К каждому конкретному экземпляру можно приклеить доплнительные атрибут:
b.leg = 5  # указание кол-ва ног у Зорьки.. Но #* Делать так нельзя, хоть язык
# ? и позволяет, но нарушается принцип инкапсуляции.
...
class Student:
    LEGS = 2  # ? константа класса, общая для всех его объектов (может меняться
    ...
    # ? большими буквами (на самом деле это только для себя, чтобы не путать)
...
"""  # ? Если при создании через конструктор __init__ не указать обязательные
...
class Persona:  # ?создание класса с конструктором __init__
    ...
    def __init__(self, name, age):
        ...
        # классе счетчик увеличивает свое значение
...
class Complex:  # Странный конечно пример, на методе составных чисел
    ...
    def __new__(cls, *args, **kwargs):
        ...
        экзепляров этого класса. Его станартный функционал выглядит так:"""
    ...
    def __init__(self, real=0.0, imreal=0.0):
        """Универсальный конструктор объектовых аттрибутов.
        Данный метод вызывается автоматически при создании экземпляра класса
        для иниициализации изначального состояния данного объекта. задает его
...
Метод __set__ переопределяет какое-то поведение, если мы попытаемся в наш
атрибут что-то присвоить, а метод __delete__ будет говорить о том, что будет
происходить, если мы удалим наш атрибут.
...
Rласс является дескриптором при переопределении любого из трех методов. Если
...
class Descriptor:
    ...
    например, чтобы значение value в методе __set__ умнодалось на 10.., или
    котрый будт записывать все переменные перед удалением в файл и т.п. """
...
"""на основе дескрипторов написано большинство стандартных функций и
декораторов (property, classmethod, staticmetod) и с их помощью могут быть
переопределены.
с помощью дескрипторов в Python реализовано очень много чего, и, например, есть
такая конструкция __slots__, которая работает тоже с помощью дескрипторов.
__slots__ позволяет вам определить класс, у которого есть жестко заданный набор
...
class Slots:
    ...
    Собственно, он при инициализации и создается. Если попытатья добавить в
    класс, или в объект какой-то еще один атрибут, то ничего не получится,
    потому что у класса нет, справочника, нет dict, в который это записывается
    """
    __slots__ = ['anakin']
...
Основан на функциональности замыкания.
Позволяют соблюдать принципы SOLID (разделять ответственность), KISS и т.п.
Если неоюходимо считать время исполнения у множества функций, в каждую из них
пришлось бы вписывать счетчик времени, а через декоратор соблюдается чистый код
+ идет разделение ответственности (функция делает свое дело, декоратор свое)
...
deco = one(10**5)  # вызов функции как обычно, но она обернута декоратором @ и
...
"""#//Применяется для создания методов класса. Gервый рагумент не self а cls
...
#? Атрибуты с _g на конце, это глобальные переменные, пример attrib_g
Атрибуты БОЛЬШИМИ_БУКВАМИ - это #? глобальные константы, пример ATTRIBUT
...
#? Приватные поля __attr - не наследуются
"""
...
# ? создание доступа по методу свойства ------------------------
...
#? Принцип Барбары Рыльска. Потомок не может не делать того, что умеет родитель
...
#? super().method()... Эта запись идентична:
#? super(Chaild_class, self).method(), где metod - метод родительского класса.
...
class ChaildClass(ParientClass):
    def method(self):
        ...
        Base.method(self)
        """ При обращении по методу super(), обращение идет по цепочке MRT """
...
"""
# ? Полиморфизм это способность одинаковым образом обрабатывать данные разных
...
Библиотека для абстрактных классов в Python - abc"""
